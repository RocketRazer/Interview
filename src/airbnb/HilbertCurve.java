...
public class HilbertCurve {
    public int hc(int x, int y, int iter) {
        ...
        if (x < len && y < len) {
            // I Clock-wise rotate 90
            return hc(y, x, iter - 1);
        } else if (x < len && y >= len) {
            // II Shape is identical with previous iteration
            return areaCount + hc(x, y - len, iter - 1);
        } else if (x >= len && y >= len) {
            // III Shape is identical with previous iteration
            return 2 * areaCount + hc(x - len, y - len, iter - 1);
        } else {
            // IV 4 Anti-Clockwise rotate 90
...
//        System.out.println(hc.hc(0,1,1)); //2
//        System.out.println(hc.hc(0,1,1)); //2
//
//
//        System.out.println(hc.hc(1,1,2)); // 3
//        System.out.println(hc.hc(2,2,2)); // 9
//        System.out.println(hc.hc(1,3,2)); // 7
        System.out.println(hc.hc(3,1,2)); // 13
    }
}
